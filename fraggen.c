...
void parse_thing_common(char *left,struct thing *t)
{
  ...
  */
  ...
  case 'z':
    ...
    if (debug_deref) printf("deref from zp\n");
    t->deref++;
  case 'c':
    t->name=&left[3];
    break;
  ...
  if (strchr(t->name,'_')) {
	...
	if (debug_deref) printf("deref due to derefidx\n");
	t->deref++;
	suffix=NULL;
      }
    }
...
struct thing *parse_thing(char *left)
{
  ...
  if (!strcmp(left,"vbuaa")) {
    ...
    free(t);
    t=t2;
    if (debug_deref) printf("deref via re-write\n");
    t->early_deref++;
    t->deref++;
...
void expand_op(int byte,struct thing *r,char *name_in_y,int reg_a_in_y)
{
  ...
  if (r->arg_thing) {
	...
	break;
      case 1:
	switch(byte) {
	case 0: printf("adc %s%s\n",name,suffix); break;
	case 1: printf("adc %s+1%s\n",name,suffix); break;
	case 2: printf("adc %s+2%s\n",name,suffix); break;
	case 3: printf("adc %s+3%s\n",name,suffix); break;
	}
	break;
      case 2:
	switch(byte) {
...
int generate_assignment(char *left, char *right,int comparison_op,char *branch_target)
{
  ...
  */
  ...
  if (left[0]=='p'&&left[1]=='p'&&(!strncmp(right,"_ptr_",5))) { l->bytes=2; }
  ...
  if (l->deref>1||r->deref>1) {
	...
	y_zero=reverse_order?0:1;
      }
      break;
    case 1:
      {
	int show=0;
	if ((l->derefidx&&!l->derefidx->reg_y)) show=1;
	if (!l->derefidx) show=1;
	if ((r->derefidx&&!r->derefidx->reg_y)) show=1;
	if (!r->derefidx) show=1;
	if (show) {
	  if (r->derefidx&&r->derefidx->name)
	    printf("ldy #{%s}\n",r->derefidx->name);
	  else {
	    printf("ldy #%d\n",reverse_order?(valid_bytes-1):0);
	    y_zero=reverse_order?0:1;
	...
	if (r->reg_a) {
	  ...
	  //	  describe_thing(0,l);
	  if (0) printf("l->pointer=%d, l->deref=%d, r->pointer=%d, l->early_deref=%d,l->post_deref=%d\n",
			l->pointer,l->deref,r->pointer,l->early_deref,l->post_deref);
	  //	  describe_thing(0,l);
	  if (l->pointer==2&&l->post_deref) {
	    if (!l->derefidx->reg_y) {
	      if (l->derefidx->reg_x) {
		  printf("stx $ff\nldy $ff\n");
		inner_deref_done=1;
  ...
  if (r->inc==1&&!strcmp(r->name,l->name)
	...
	if (r->shift<0) {
	  ...
	  case 2:
	    printf("ldy #%d\n",l->bytes-1);
	    if (l->bytes-1) y_zero=0; else y_zero=1;
	    for(int b=l->bytes;b;b--) {
	      if (b==l->bytes)
